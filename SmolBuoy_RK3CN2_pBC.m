...
for i = 1:nsteps
    ...
    Wrhs_coeff=-WK2/alpha(k)*W_profile-Wlap_coeff+Re/alpha(k)*(gamma(k)*Wadv_p_coeff);
    W_profile= Wrhs_coeff/(Rd2x-WK2/alpha(k)*eye(N_mesh));
...
%         adv_p_coeff=adv_coeff+swim_coeff;
    ...
    Wrhs_coeff=-WK2/alpha(k)*W_profile-Wlap_coeff+Re/alpha(k)*(gamma(k)*Wadv_comb_coeff+rho(k)*Wadv_p_coeff);
    W_profile= Wrhs_coeff/(Rd2x-WK2/alpha(k)*eye(N_mesh));
    S_profile= W_profile*Rdx/2;
...
%     end
    ...
    if ( mod(i, saving_rate3) == 0 )
        ...
        W_prof(i/saving_rate3,:)=W_profile;
